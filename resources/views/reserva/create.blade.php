...
@section('header', 'Reserva agregar')

@section('content')

<form onsubmit="return validateForm()" method="POST" action="{{route('reserva.store')}}">
    @csrf
    ...
    <div class="container" style="margin-top:30px">
        <div class="row">
            <div class="col-md-4">
                ...
                <div class="form-group">
                    <label for="tipor">Tipo reserva</label>
                    <select id="tipor" name="tipo_reserva" required tabindex="4">
                        <option value="">--Selecciona una opción--</option>
                        @foreach($tipo_reservas as $tipo_reserva)
                        <option value={{ $tipo_reserva->id }}>{{ $tipo_reserva->descripcion }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-10">
                    <div class="d-flex flex-wrap justify-content-md-around ">
                        ...
                        <div class="p-2 mx-auto"><button type="button" onclick="location.href = '{{ route('reserva.index') }}'" class="btn btn-dark">SALIR</button></div>
                    </div>
                </div>
            </div>
        </div>
