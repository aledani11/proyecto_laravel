...
@section('header', 'Entrada agregar')

@section('content')

<form onsubmit="return validateForm()" method="POST" action="{{route('entrada.store')}}">
    @csrf
    ...
    <div class="container" style="margin-top:30px">
        ...
        <div class="form-group my-2 col-md-10">
            <h5>Entrada Detalle</h5>
        </div>
        <div class="row">
            <div class="col-md-10 form-group">
                <div class="table-responsive-md table-hover from-group" style="overflow-y:auto; height:200px">
                    <table id="entrada_table" data-toggle="table" data-classes="table table-bordered table-hover table-md">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Articulo</th>
            ...
            <div class="col-md-2 form-group">
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" onclick="add('entrada')" tabindex="8">Agregar</button>
                </div>
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" onclick="remove('entrada')">Quitar</button>
                </div>
                <div class="col-md-12">
                    <button type="button" class="btn btn-dark form-group" style="display: none;" onclick="edit('entrada')">Editar</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-md-10">
                <div class="d-flex flex-wrap justify-content-md-around ">
                    ...
                    <div class="p-2 mx-auto"><button type="button" onclick="location.href = '{{ route('entrada.index') }}'" class="btn btn-dark">SALIR</button></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    var id_delete = null;
    $('#entrada_table, #cheque_table').on('click-row.bs.table', function(e, row, $element, field) {
        id_delete = row;
    ...
    $('#entrada_table, #cheque_table, #habitacion_huespedes').on('click', 'tbody tr', function(event) {
        $(this).addClass('highlight').siblings().removeClass('highlight');
    });

    function remove_all() {
        $("#entrada_table").bootstrapTable('removeAll');
    ...
    function remove(table_remove) {
        ...
        //console.log(combo_habi_edit);
        if (table_remove === "entrada") {
            var table_rem = "#entrada_table";
    ...
    function validateForm() {
        ...
        if (taf.length == 0) {
            alert("Cargar al menos una entrada detalle");
            return false;
        }
        //console.log(taf);
